...
```bash
cd suite/suite_meet/sfu-server
yarn install
cp .env.example .env
```
...
Replace `suite.localhost` with your site name, then return to `apps/suite/suite/suite_meet/sfu-server` and start the SFU:
...
curl -fsSL https://raw.githubusercontent.com/frappe/suite/develop/suite/suite_meet/sfu-server/deploy/install.sh | bash
