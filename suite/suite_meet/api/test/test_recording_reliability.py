...
from suite.suite_drive.api.files import delete_entities, remove_or_restore
from suite.suite_drive.utils import create_drive_file, get_user_folder
from suite.suite_drive.utils.files import TRASH_PREFIX, FileManager
...
from suite.suite_meet.patches.backfill_recording_finalization import execute as backfill_recording_finalization
...
class IntegrationTestRecordingReliability(IntegrationTestCase):
    ...
    def test_estimate_contract_for_ad_hoc_scheduled_and_recurring_rooms(self):
        ...
        for label, event, expected in cases:
            with (
                ...
                patch("suite.suite_meet.api.recording.frappe.get_doc", return_value=event),
                patch("suite.suite_meet.api.recording.now_datetime", return_value=fixed_now),
            ):
    ...
    def test_budget_eligibility_boundaries(self):
        for free_bytes, eligible in (
            ...
            with (
                ...
                patch("suite.suite_meet.api.recording._get_free_bytes", return_value=free_bytes),
                patch("suite.suite_meet.api.recording._get_drive_destination", return_value="home"),
    ...
    def test_real_drive_trash_restore_and_permanent_delete_removes_blob_and_metadata(self):
        ...
        append_chunk(recording.name, offset=0, chunk=content, chunk_sha256=digest)
        complete_upload(recording.name, event_sequence=7)
        with patch("suite.suite_meet.recording.ingest._validate_media", return_value={"duration_ms": 1000}):
            result = process_upload(recording.name)
        artifact = frappe.get_doc("File", result["artifact"])
