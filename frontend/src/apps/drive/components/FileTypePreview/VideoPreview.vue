...
const src = ref(
  `/api/method/suite.suite_drive.api.files.stream_file_content?entity_name=${props.previewEntity.name}`
)
const type = ref(
...
watch(
  ...
  (newValue) => {
    loading.value = true
    src.value = `/api/method/suite.suite_drive.api.files.stream_file_content?entity_name=${newValue.name}`
