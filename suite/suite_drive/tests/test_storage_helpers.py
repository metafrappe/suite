...
from suite.suite_drive.overrides.file import File
from suite.suite_drive.utils import WRITER_CONTENT_DOCTYPE
...
class TestStorageHelpers(unittest.TestCase):
    ...
    def test_writer_container_follows_rename_trash_and_restore(self):
        with TemporaryDirectory() as site_folder:
            ...
            with patch(
                "suite.suite_drive.utils.files.get_root_folder", return_value={"file_url": "private/files/root"}
            ):
    ...
    def test_s3_root_paths_strip_disk_prefixes(self):
        ...
        for root_url in [
            ...
            with self.subTest(root_url=root_url):
                with patch("suite.suite_drive.utils.files.get_root_folder", return_value={"file_url": root_url}):
                    self.assertEqual(
    ...
    def test_local_root_path_keeps_disk_prefix(self):
        ...
        with patch(
            "suite.suite_drive.utils.files.get_root_folder",
