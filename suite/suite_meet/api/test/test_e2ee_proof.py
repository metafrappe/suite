...
class IntegrationTestE2EEEpoch(IntegrationTestCase):
    ...
    def test_register_e2ee_device_recovers_from_concurrent_insert_race(self):
        ...
        with (
            patch(
                "suite.suite_meet.api.meeting.frappe.db.get_value",
                side_effect=[None, "existing-row"],
            ),
            patch("suite.suite_meet.api.meeting.frappe.new_doc", return_value=fake_doc),
            patch("suite.suite_meet.api.meeting.frappe.db.set_value") as set_value,
        ):
