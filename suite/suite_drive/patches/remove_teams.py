...
from suite.suite_drive.utils.files import TRASH_PREFIX, FileManager, storage_key
...
def execute():
    ...
    frappe.enqueue(
        "suite.suite_drive.patches.remove_teams.sweep_sidecars",
        queue="long",
        timeout=4 * 60 * 60,
        sidecars=sidecars,
        trashed=trashed,
...
def sweep_sidecars(sidecars=None, trashed=None):
    ...
    Runs as a job; safe to re-run by hand if it failed:
        ...
        bench --site <site> execute suite.suite_drive.patches.remove_teams.sweep_sidecars
