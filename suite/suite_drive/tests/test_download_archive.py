...
from suite.suite_drive.api.files import _collect_download_files
from suite.suite_drive.utils import STATUS_ACTIVE, STATUS_TRASHED


class TestDownloadArchive(unittest.TestCase):
    @patch("suite.suite_drive.api.files.user_has_permission", return_value=True)
    ...
    def test_collect_files_enforces_permission(self):
        with patch("suite.suite_drive.api.files.user_has_permission", return_value=False):
