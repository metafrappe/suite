...
from suite.suite_drive.api.files import (
    SEARCH_PAGE_LENGTH,
    create_auth_token,
    does_entity_exist,
    get_file_content,
...
from suite.suite_drive.api.list import get_attachments
...
from suite.suite_drive.patches.normalize_attachment_file_types import execute as normalize_attachment_file_types
...
from suite.suite_drive.utils.files import FileManager, get_s3_url
...
class TestDriveFilesAPI(IntegrationTestCase):
    ...
    def test_track_visit_resolves_backing_file(self):
        ...
        with (
            self.set_user(OWNER),
            patch("suite.suite_drive.api.files.mark_as_viewed") as mark_as_viewed,
            patch("suite.suite_drive.api.files.frappe.db.set_value"),
        ):
    ...
    def upload_request(self, content, filename, session, chunk=None):
        builder = EnvironBuilder(
            path="/api/method/suite.suite_drive.api.files.upload_file",
            method="POST",
    ...
    def upload(self, content, filename="upload.txt", session=None, chunk=None, total_size=None):
        ...
        with (
            ...
            patch("suite.suite_drive.api.files.validate_quota"),
            patch("suite.suite_drive.api.files.frappe.publish_realtime"),
        ):
            return upload_file(
    ...
    def test_upload_rejects_absolute_session_before_writing(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                self.set_user(OWNER),
                patch("suite.suite_drive.api.files.frappe.get_site_path", return_value=str(storage_root)),
                patch(
                    "suite.suite_drive.api.files.frappe.get_single",
    ...
    def test_upload_removes_temp_file_on_quota_rejection(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch(
                    "suite.suite_drive.api.files.validate_quota",
    ...
    def test_content_link_cannot_be_forged_to_hijack_another_users_document(self):
        ...
        content_has_permission in suite/suite_drive/overrides/file.py): whoever's
    ...
    def test_download_token_is_single_use_sequentially(self):
        ...
        with (
            self.set_user("Guest"),
            patch(
                "suite.suite_drive.api.files.get_file_internal", return_value=b"file contents"
    ...
    def test_trash_and_restore_preserve_status(self):
        with (
            self.set_user(OWNER),
            patch("suite.suite_drive.api.files.validate_quota"),
            patch("suite.suite_drive.api.files.FileManager.move_to_trash") as move_to_trash,
            patch("suite.suite_drive.api.files.FileManager.restore") as restore,
        ):
...
class TestDriveSearch(IntegrationTestCase):
    ...
    def test_reaches_readable_rows_past_the_first_window(self):
        ...
        with (
            self.set_user(OTHER_USER),
            patch("suite.suite_drive.api.files.SEARCH_SCAN_WINDOW", self.WINDOW),
        ):
    ...
    def test_scan_stops_at_the_budget(self):
        ...
        with (
            self.set_user(OTHER_USER),
            patch("suite.suite_drive.api.files.SEARCH_SCAN_WINDOW", self.WINDOW),
            patch("suite.suite_drive.api.files.MAX_SEARCH_SCAN_WINDOWS", 2),
            patch("suite.suite_drive.api.files.user_has_permission", return_value=False) as has_permission,
        ):
            self.assertEqual(search(self.token), [])
    ...
    def test_stops_once_the_page_is_full(self):
        with (
            self.set_user(OWNER),
            patch("suite.suite_drive.api.files.SEARCH_PAGE_LENGTH", 3),
            patch("suite.suite_drive.api.files.SEARCH_SCAN_WINDOW", self.WINDOW),
        ):
    ...
    def test_access_is_resolved_without_reloading_each_row(self):
        ...
        with (
            self.set_user(OWNER),
            patch("suite.suite_drive.api.files.user_has_permission", return_value=True) as has_permission,
        ):
            search(self.token)
    ...
    def test_blank_query_short_circuits(self):
        with self.set_user(OWNER), patch("suite.suite_drive.api.files.frappe.db.sql") as sql:
            for query in ("", "   ", "\t\n"):
