...
from suite.suite_meet.recording.ingest import InfrastructureFinalizationError, _validate_media
    ...
    def test_probe_and_decode_failures_are_classified(self):
        for error in (subprocess.TimeoutExpired("ffprobe", 120), OSError("ffprobe unavailable")):
            with (
                ...
                patch("suite.suite_meet.recording.ingest.subprocess.run", side_effect=error),
        ...
        with (
            patch(
                "suite.suite_meet.recording.ingest.subprocess.run",
        ...
        for output in ("not-json", json.dumps({"streams": "invalid"}), "x" * (1024 * 1024 + 1)):
            with (
                ...
                patch("suite.suite_meet.recording.ingest.subprocess.run", return_value=Mock(stdout=output)),
                self.assertRaises(frappe.ValidationError),
            ):
                _validate_media(Path("recording.mp4"))
